...
class Maps:
    ...
    def populateDifficulty(
        self, gameMode: str, gameModeList: List[str], difficulty: str
    ) -> None:
        for mode in gameModeList:
            self.tree.insert(
                ...
                "end",
                text=mode,
                tags=[self.tree.item(self.map, "text"), difficulty, mode],
            )
    ...
    def options(self, tags: List[str]) -> None:
        ...
        if mode == "Standard":
            round40: Button = Button(
                self.frame,
                text="Round 40 restart",
                command=lambda: self.round40(name, difficulty),
            )
            round100: Button = Button(
                self.frame,
                text="Round 100 restart",
                command=lambda: self.round100(name, difficulty),
            )
